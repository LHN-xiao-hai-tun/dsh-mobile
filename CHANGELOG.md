...
> **应用本身没有功能变化**（与 1.3.10 行为一致）—— 这一版是**发版链路升级 + 验证**：
> 从「本地签名发版」升级为「**打 tag → 云端签名 → 自动发 Release**」，并按安全性加了三道闸。
> 顺带给手机端 Obtainium 一个可用于验证"能收到更新"的新版本号。

### 变更（发布流程 · CI）

- 🔑 **CI 云端签名（A 路启用）**：签名密钥存于 GitHub **Environment `release` 的环境级 Secrets**
  ⇒ 打 tag 后 CI 直接产出**已签名**的规范名资产 `dsh-mobile-vX.Y.Z.apk` + `.apk.sha256`，
  **不再需要本机参与**（本地 `scripts\release.ps1` 仍保留为备用路径）。
- 🛡 **三道加固**（同批启用）：
  1. **发布需人工批准**：`environment: release` 配置了 Required reviewers（= 仓库 owner）且
     `can_admins_bypass=false` ⇒ **没点批准，签名作业不执行、密钥读不到**
     （这是"workflow/tag 被改也签不出来"的那道闸；批准页手机浏览器即可打开）。
  2. **action 全部钉到 commit SHA**（仓库设置 `sha_pinning_required=true` 强制）⇒ 防"移动的 tag"投毒。
  3. **tag / 分支保护规则**：`v*` 与默认分支只允许 owner 改动。
- 🔴 **签名指纹自检**：CI 在发布前断言 APK 证书指纹 == 仓库变量 `EXPECTED_CERT_SHA256`，
  **不符则失败**（换钥匙 = 老用户无法覆盖安装）；未签名的包一律拒绝发布。
- 📝 **Release Notes 由 CI 生成**：从本文件抽取对应版本段落 + 自动追加校验表
  （版本 / 最低 Android / tag→commit / 体积 / SHA-256 / 签名状态）；**抽不到版本段落即失败**。

### 降级 / 回滚

- 删掉那 4 个环境级 Secret ⇒ 发布作业**自动退回**「未签名占位 + 本地签名替换」的旧路径（B 路），
  **不需要改代码**。

### 兼容

- 签名密钥未变（证书指纹 `d787b838…c33e`）⇒ **覆盖安装即可**；权限清单未变（仍 5 条）。
...
#### ✅ D1/D2 真机补验（2026-09-23 · 平板 CHG-W60 · Android 16 · **真相机走完整链路**）

> 背景：模拟器自带相机**忽略 `EXTRA_OUTPUT`**（把照片写进它自己的相册），所以"照片真的回传到网页"
> 这一步在模拟器上跑不通；改到**平板上用真相机**跑完整链路。

| 判据 | 结果 |
|---|---|
| 网页里带 `capture` 的输入框 ⇒ 直接拉起**真相机** | ✅ 华为相机应用前台，界面元素 `点击拍照` / `确定` / `取消` |
| 拍完 → 「确定」 ⇒ 回到 App | ✅ 回到测试页 |
| **照片真的交到网页** | ✅ 页面显示 **`已收到: shot-1790136776274.jpg / 556313 B`**（543 KB · 文件名正是我们 FileProvider 临时目录里的 `shot-<毫秒>.jpg`） |
| 截图存证 | ✅ `02_产物\dsh-mobile\证据\真机截图_v1.3.9_D2拍照回传成功_2026-09-23.png` |
| 验完把平板地址**恢复原值** | ✅ 切回 `http://192.168.10.44:3082`，主界面重新加载平板侧真实 DSH 页（`总会话4.4.5 — DSH 本地构建`） |
...
> 附带证据：`02_产物\dsh-mobile\证据\模拟器截图_v1.3.9_连接诊断_全绿_dsh-26g.png`
...
| **自签 https · 无指纹时应拒绝** | ❌ **未验到** —— 隔离手法失效：跨协议链接（http 页 → https 资源）上的 `download` 属性会被浏览器忽略 ⇒ WebView 直接**导航**过去（弹的是**页面自身**的 TOFU 框），**没走到下载路径** |
...
- **网页下载不再"点了没反应"**：弹一个选择 —— **用系统浏览器打开** / **复制链接**。
  - ⛔ **本版刻意不替用户取文件、不落盘**。三条理由（详见 `03_文档\方案与评估\设计_dsh-mobile_上传下载_2026-09-23.md` §四）：
